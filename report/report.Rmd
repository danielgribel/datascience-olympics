...
o que significa que em média um país sede aumenta em 1.88x o seu *score*. Portanto, nesse ponto já temos condições de fazer uma primeira previsão (ainda grosseira) para o Brasil. 

Pegamos o desempenho do país nas últimas 6 olímpiadas:
```{r eval=FALSE}
...
Obtemos $r$, como sendo a proporção calculada acima:
```{r eval=FALSE}
# ratio: mean between home factor and mean away factor
r <- mean(home_factor/mean_away_factor)
```

E fazemos a predição do score para o Brasil em 2016:
```{r eval=FALSE}
