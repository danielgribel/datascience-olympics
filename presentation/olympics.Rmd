...
p <- pred_bra_score*(1/3) + pred_bra_score*(1/6) +
  pred_bra_score*(1/9)
...
```{r echo=FALSE}
cor <- round(cor_gdp, 4)
cor_bra <- round(cor(country_medals_ratio("Brazil"), ind_factor["Brazil",]), 4)
colnames(cor) <- hosts_acronym
colnames(cor_bra) <- "BRA"
```
```{r eval=FALSE}
cor_gdp
```
